...
XSMB (Miền Bắc) lottery analytics & statistical prediction engine — Python + PostgreSQL.

Migrate từ stack Java/MongoDB (`analysis-rbk`) sang FastAPI/Postgres, bổ sung prediction engine với walk-forward backtest và ensemble tuning.
...
app/
  ...
  prediction/             # Prediction engine (M1–M8)
    features.py           # Load history, feature context
    models/               # frequency, ewma, gap, markov, bayesian, weekday, digit
    ensemble.py           # M8 combiner
    backtest.py           # Walk-forward backtest
    tuning.py             # Ensemble weight search
    service.py            # Orchestration
    weights.py            # Load tuned_weights.json
    tuned_weights.json    # Trọng số đã tune (2020–2025)
  repositories/           # draw, kqxs, prediction, rbk, user
  routers/                # kqxs, rbk, analytics, predictions, ums
  services/               # scrape, import, legacy kqxs
db/
...
| `RBK_ENABLE_SCHEDULER` | `true` | Cron import + predict |
...
| Method | Endpoint | Mô tả |
|--------|----------|--------|
| GET | `/predictions/next` | Dự đoán ngày tiếp (`target=loto\|de\|dau\|dit`) |
| GET | `/predictions/weights` | Trọng số tuned + metrics |
| GET | `/predictions/evaluate` | So sánh predict vs thực tế (`?date=`) |
| POST | `/predictions/backtest` | Walk-forward backtest |

Query mẫu:

```bash
curl "http://localhost:8081/predictions/next?target=loto&top=20&model=ensemble"
curl "http://localhost:8081/predictions/evaluate?date=2024-08-09&target=loto&top=20"
```
...
| 18:15+ (sau import OK) | Auto predict ngày mai | `compute_next()` |
| 19:05 | Predict dự phòng | Nếu import 18h fail |
| 19:00 | RSS MN/MT | |
...
- [SPEC Prediction Engine](docs/SPEC-prediction-engine.md) — thuật toán, schema, API chi tiết, implementation status
