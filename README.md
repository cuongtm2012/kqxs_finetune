...
Migrate từ stack Java/MongoDB sang FastAPI/Postgres. API chính hiện tại là **Stats Engine** (thống kê mô tả + candidate pool). Code prediction engine (`app/prediction/`) vẫn giữ để tham khảo nhưng **không còn mount router / scheduler**.
...
| Layer | Công nghệ |
|-------|-----------|
| API | FastAPI + Uvicorn (port `18715` mặc định, extension forum) |
...
APP_PORT=18715 .venv/bin/python run.py
...
| Biến | Mặc định | Mô tả |
|------|----------|--------|
| `APP_PORT` | `18715` | API port |
| `APP_DATABASE_URL` | `postgresql://rbk:rbk@127.0.0.1:5436/rbk` | Postgres |
| `APP_ENABLE_SCHEDULER` | `true` | Cron import KQXS (không còn auto-predict) |

### Backfill lịch sử

```bash
.venv/bin/python scripts/backfill_xsmb.py --from 2007-01-01 --delay 0.3
curl -X POST http://localhost:18715/analytics/refresh-views
```

### Forum Intelligence (`/forum/*`) + Chrome extension

Chrome extension `extension/` thu thập pick từ forumketqua.net → sync API.

| Method | Endpoint | Mô tả |
|--------|----------|--------|
| POST | `/forum/picks` | Ingest full `CollectSession` từ extension |
| GET | `/forum/picks/{date}` | Session đã lưu |
| GET | `/forum/experts/live` | Cao thủ đang chốt + weight |
| GET | `/forum/recommendations` | Đề xuất **forum-only** (khớp tab Đề xuất extension) |
| GET | `/forum/experts/backtest` | Backtest pick vs KQ XSMB |

```bash
# API + extension
APP_PORT=18715 .venv/bin/python run.py
cd extension && npm run build   # load extension/dist/ trong Chrome

# Báo cáo ngày — forum-only (giống extension)
PYTHONPATH=. .venv/bin/python scripts/xsmb_daily_report.py --source api --mode forum

# Báo cáo hybrid engine+forum (mặc định)
PYTHONPATH=. .venv/bin/python scripts/xsmb_daily_report.py --source api --mode hybrid

# Backtest trọng số cao thủ
PYTHONPATH=. .venv/bin/python scripts/backtest_expert_picks.py --days 90
```

Chi tiết OpenSpec: `openspec/changes/forum-intelligence-v1/`, `extension/openspec/`.
...
| GET | `/stats/intersection` | **v4** CF ∩ cầu lặp (`min_cf_lift`, `min_rbk_cau`) |
| GET | `/stats/intersection/evaluate` | Đánh giá intersection vs KQXS 1 ngày (đề loto) |
| POST | `/stats/intersection/backtest` | Backtest intersection + so sánh CF/cầu lặp alone |
...
## Scheduler (khi `APP_ENABLE_SCHEDULER=true`)
...
9. **Gap model (M3)** về lý thuyết là gambler's fallacy — giữ vì legacy cũ.
